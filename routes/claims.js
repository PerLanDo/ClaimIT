  ...
  async (req, res) => {
    try {
      ...
      if (item.status !== "active") {
        return res
          .status(400)
          .json({ error: "Item is not available for claiming" });
      }
      ...
      if (item.poster_id === req.user.id) {
        return res
          .status(400)
          .json({ error: "You cannot claim your own item" });
      }
      ...
      if (claimError && claimError.code !== "PGRST116") {
        ...
        return res
          .status(500)
          .json({ error: "Failed to check existing claims" });
      }
      ...
      // Upload proof image to Backblaze B2
      ...
      if (req.file) {
        try {
          ...
          if (uploadResult.success) {
            ...
            console.error("Proof image upload failed:", uploadResult.error);
  ...
  async (req, res) => {
    try {
      ...
      if (claim.status !== "pending") {
        return res
          .status(400)
          .json({ error: "Claim has already been reviewed" });
      }
