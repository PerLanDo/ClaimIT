...
app.use("/api/test", require("./routes/test")); // Test endpoints for B2 integration
...
app.post("/api/mcp/setup-database", async (req, res) => {
  try {
    const supabase = mcpManager.getSupabase();
    const result = await supabase.createTables();
...
// Check if database tables exist endpoint
app.get("/api/mcp/check-tables", async (req, res) => {
  try {
    const supabase = mcpManager.getSupabase();
    const result = await supabase.checkTablesExist();
